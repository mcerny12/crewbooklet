...
describe('calculateDay — per diem auto threshold (Bug #3)', () => {
  ...
  it('auto: exactly 8h actual → partial per diem', () => {
    const r = calculateDay(
      day('2026-07-13', '08:00', '16:00', { perDiemType: 'auto', placeOfWork: 'Hamburg' }),
      BASE_RULESET
    );
    expect(r.totalWorkMinutes).toBe(480);
  ...
  it('full → full per diem €28 regardless of hours', () => {
    const r = calculateDay(
      day('2026-07-13', '08:00', '16:00', { perDiemType: 'full', placeOfWork: 'Hamburg' }),
      BASE_RULESET
    );
  ...
  it('none → 0', () => {
    const r = calculateDay(
      day('2026-07-13', '08:00', '16:00', { perDiemType: 'none', placeOfWork: 'Hamburg' }),
      BASE_RULESET
    );
...
describe('calculateDay — home base per diem suppression (Bug #2)', () => {
  ...
  it('null place of work defaults to home base → per diem suppressed, not awarded by omission (Bug #7)', () => {
    const r = calculateDay(
  ...
  it('empty-string place of work also defaults to home base → per diem suppressed', () => {
    const r = calculateDay(
      day('2026-07-13', '08:00', '18:00', { perDiemType: 'auto', placeOfWork: '   ' }),
      BASE_RULESET
    );
    expect(r.perDiemCents).toBe(0);
  });
